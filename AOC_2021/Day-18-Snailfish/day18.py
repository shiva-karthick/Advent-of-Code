...
def explode(x):
    for i, ((num1, depth1), (num2, depth2)) in enumerate(zip(x, x[1:])):
        if depth1 < 5 or depth1 != depth2:
            continue
        # the 2 if statements execute this --> "To explode a pair, the pair's left value is added to the rightmost regular number to the left of the exploding pair (if any), and the pair's right value is added to the leftmost regular number to the right of the exploding pair (if any)"
        if i > 0:
            x[i-1][0] += num1
        if i < len(x)-2:  # meaning we are at the start of the list and there exists no element on the left
            x[i+2][0] += num2
        #
...
start = pfc()
print(solve(read_puzzle('AOC_2021\Day-18-Snailfish\sample.txt')))
print(pfc()-start)
