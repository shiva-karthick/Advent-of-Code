...
def read_input(path: str = 'AOC_2022\Day-16-Proboscidea-Volcanium\\test_input.txt'):
    inputs = dict()
    with open(path) as filet:
        for line in filet.readlines():
            ...
            next_elements = line.split('valve')[1].replace(
                's ', '').replace(' ', '').split(',')
...
def get_worthy_valves(inputs: dict):
    ...
    non_zero.add('AA')  # becasue we need to start from AA
    return non_zero


def get_shortest_connections(worthy_valves: set, graph: dict):  # non_zero, inputs
    ...
    for idx, start in enumerate(non_zero_list):
        for end in non_zero_list[idx + 1:]: # make sure the start and end are not the same
            # get the shortest path
...
def traverse_everything(shortest_path, graph, time):
    ...
    while queue:
        ...
        for neighbor in neighbours:
            # get the new flow
            new_flow = (
                time - shortest_path[position][neighbor] - 1) * graph[neighbor][0]

            # make the new set
            new_set = visited | {neighbor}
            queue.append((neighbor, accumulated_flow + new_flow,
                         time - shortest_path[position][neighbor] - 1, new_set))
    return pathes
