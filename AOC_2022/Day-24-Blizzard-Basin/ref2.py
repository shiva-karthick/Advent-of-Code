...
for y, line in enumerate(open("input.txt")):
    for x, c in enumerate(line):
        if c == '#':
            W.add((x - 1, y - 1))
        if c == '>':
            B.add((x - 1, y - 1, +1, 0)) # go to right
        if c == '<':
            B.add((x - 1, y - 1, -1, 0)) # go to left
        if c == '^':
            B.add((x - 1, y - 1, 0, -1)) # go down
        if c == 'v':
            B.add((x - 1, y - 1, 0, +1)) # go up
...
while goals:
    ...
    q = n - b - W # why subtract?
    # print(t, len(q), goals[0])
    if goals[0] in q:
        print(
            f"goal {goals[0]} reached after {t} steps (queue size: {len(q)})")
        q = {goals.pop(0)}
